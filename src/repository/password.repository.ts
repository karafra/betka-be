...
export class PasswordRepository {
  ...
  async getPasswordByCrackTime(crackTime: number, greater = false) {
    this.logger.debug(`Retrieving passwords with worse crack time than ${crackTime}`);
    const passwords = await this.passwordModel.find(
      {
  ...
  async getPage(startIndex: number, endIndex: number) {
    ...
    this.logger.debug(`Retrieving ${pageSize} passwords (startIndex: ${startIndex} endIndex: ${endIndex}`);
  ...
  async getTopBy(parameterName: string, amount: number) {
    this.logger.debug(`Retrieving top ${amount} passwords by parameter '${parameterName}`);
    const filter = {};
    ...
    const agregation = await this.passwordModel
      .aggregate([
        {
          '$match': {
            'globalRank': {
